...
##### Code definition: Beta diversity analysis (FecesOnly subgroup)
...
beta_cfg <- config$beta_diversity_fecesonly
...
subset_col <- beta_cfg$subset$subset_col
subset_value <- beta_cfg$subset$subset_value
...
random_seed <- beta_cfg$random_seed
...
common_samples <- intersect(colnames(abundance_mat), metadata[[sample_id_col]])
...
pcoa_var <- round(100 * pcoa_res$eig[1:ordination_k] / sum(pcoa_res$eig), 1)
...
message("Beta diversity analysis (FecesOnly subgroup) completed.")
